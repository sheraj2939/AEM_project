(function ($, window) {
    ...
    FreezeTable.prototype.buildScrollBar = function () {
        ...
        this.$scrollBarWrap = $('<div class="' + this.scrollBarWrapClass + '"></div>')
            ...
            .css('bottom', 0)
            .css('z-index', 2)
            .css('width', (this.$tableWrapper.width() + 16))
    ...
    // $("#tablefixed-top-left").freezeTable({
    //     'scrollable': true,
    // });

    $("#tablefixed-top-left2").freezeTable({
        'scrollable': true,
        'freezeHead': true,
        'freezeColumn': false,
    });
    ...
    $("#tablefixed-left").freezeTable({
        'freezeHead': false,
    });

    if ($(window).width() < 1200) {
        $("#tablefixed-top").freezeTable({
            'scrollable': true,
            'freezeColumn': true,
        });
    } else {
