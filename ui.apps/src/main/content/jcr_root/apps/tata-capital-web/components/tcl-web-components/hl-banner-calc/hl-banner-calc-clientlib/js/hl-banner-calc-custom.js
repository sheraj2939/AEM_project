function renderBannerCalcResult(calcName) {
    ...
    var emi = hlEmiCalculator.calculate(readBannerInputValues(calcName));
    setBannerCalcValues(emi, calcName)
}

function setBannerCalcValues(emi, calcName) {
    try {
        if(!isFinite(emi.emiPerMonth)){
            var calculatePrice = document.querySelector('[data-calc-name=' + calcName + '] [data-field="bannerMonthlyEMI"]');
            calculatePrice.value = 0;
        }
        else if (!isNaN(emi.emiPerMonth)) {
            var calculatePrice = document.querySelector('[data-calc-name=' + calcName + '] [data-field="bannerMonthlyEMI"]');
            calculatePrice.value = Math.round(emi.emiPerMonth).toLocaleString("EN-IN");
        }
        else {
            var calculatePrice = document.querySelector('[data-calc-name=' + calcName + '] [data-field="bannerMonthlyEMI"]');
            calculatePrice.value = 0;
...
$('[yearMonth-tab-menu]').click(function (e) {
    try {
        ...
        if ($(this).parents(".banner-calc-custom").data('calc-name').includes('banner')) {
            renderBannerCalcResult("hl-banner-calc");
        }
    } catch(e) {console.log(e);}
})
var bannerCalcUrl = window.osgiConfigObj.calcApiDomain+"/web/api/mdm/export/minmaxmaster.json?calculator-label=calculator-home-loan-emi-calc";
    ...
    try {
        bannerCalcApiCall(bannerCalcUrl).then(function (res) {
            res.Master.forEach(function (element) {
                fieldNames = document.querySelectorAll('[data-calc-name="hl-banner-calc"] [data-fieldname]');
                fieldNames.forEach(function (fieldName) {
                    /* ============== EMI CALCULATORS ================= */
                    ...
                    if (fieldName.dataset.field == "tenure") {
                        ...
                        var month = $(fieldName).closest('[yearMonth-tab-wrapper]').find("[data-month-or-year='hl-banner-calcMonth']");
                        ...
                        // console.log(month)
                        var year = $(fieldName).closest('[yearMonth-tab-wrapper]').find("[data-month-or-year='hl-banner-calcYear']")
